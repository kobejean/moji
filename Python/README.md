...
```bash
cd ..
python -c "import tensorflow as tf;import moji;print(moji.zero_out([[1,2], [3,4]]).eval(session=tf.Session()))"
```
...
```
The op implementation, including both c++ and python code, goes under `moji` dir for CPU only ops, or `tensorflow_time_two` dir for GPU ops. You will want to replace either directory with the corresponding content of your own ops. `tf` folder contains the code for setting up TensorFlow pip package as an external dependency for Bazel only. You shouldn't need to change the content of this folder. You also don't need this folder if you are using other build systems, such as Makefile. The `gpu` folder contains the code for setting up CUDA libraries and toolchain. You only need the `gpu` folder if you are writing a GPU op and using bazel. To build a pip package for your op, you will also need to update a few files at the top level of the template, for example, `setup.py`, `MANIFEST.in` and `build_pip_pkg.sh`.
...
```bash
bazel build moji:python/ops/_zero_out_ops.so
```
...
Once you have built your custom op shared library, you can follow the example in [`moji/python/ops`](https://github.com/tensorflow/custom-op/tree/master/moji/python/ops), and instructions [here](https://www.tensorflow.org/extend/adding_an_op#use_the_op_in_python) to create a module in Python for your op. Both guides use TensorFlow API `tf.load_op_library`, which loads the shared library and registers the ops with the TensorFlow framework.
```python
...
You can also add Python tests like what we have done in `moji/python/ops/zero_out_ops_test.py` to check that your op is working as intended.


##### Run Tests with Bazel
To add the python library and tests targets to Bazel, please follow the examples for `py_library` target `moji:zero_out_ops_py` and `py_test` target `moji:zero_out_ops_py_test` in `moji/BUILD` file. To run your test with bazel, do the following in Docker container,

```bash
bazel test moji:zero_out_ops_py_test
```
...
#### Build with Bazel
You can find the target for pip package in the top level `BUILD` file. Inside the data list of this `build_pip_pkg` target, you want to include the python library target ` //moji:zero_out_py` in addition to the top level files. To build the pip package builder, run the following command in Docker container,
```bash
bazel build :build_pip_pkg
```
